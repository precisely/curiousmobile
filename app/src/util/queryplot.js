...
function Plot(tagList, userId, userName, plotAreaDivId, store, interactive, properties) {
	...
	*/
	...
	this.store = function() {
		...
		var plotDataStr = JSON.stringify(plotDef);
...
function PlotLine(p) {
	...
	this.appendHTML = function() {
		...
		var html = '<div id="plotline' + idSuffix + '" class="'+plotColorClass[this.color]+'">\
			...
			+ _.escape(this.name) + '</span> <span class="plotGroup">'
	...
	this.loadPlotData = function() {
		...
		u.queueJSON("loading graph data", u.makeGetUrl(method), u.getCSRFPreventionObject(method + "CSRF", {tags: JSON.stringify(this.getTags()),
